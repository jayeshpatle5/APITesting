...
public class EndToEndEcommerceAPI {
	static String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NDU0YTcxMzU2OGMzZTlmYjE2NDQyZjIiLCJ1c2VyRW1haWwiOiJqYXllc2hAZ21haWwuY29tIiwidXNlck1vYmlsZSI6MTIzNDU2Nzg5MCwidXNlclJvbGUiOiJjdXN0b21lciIsImlhdCI6MTY5NTYzODk2OCwiZXhwIjoxNzI3MTk2NTY4fQ.z7r8omNRfh1sAMcap11x8XYZUl6SzgItUg0QIdI4LDQ";
	static String userId = "6454a713568c3e9fb16442f2";
	static String productId = "65116fd87244490f95b90dab";
	...
	String res=	given().spec(reqspecificationOb()).header("Authorization", token)
		        .when().delete("/api/ecom/order/delete-order/651169d17244490f95b905a4")
		        .then().extract().response().asPrettyString();
	...
	String res=	given().spec(reqspecificationOb()).header("Authorization", token)
		       .when().get("/api/ecom/order/get-orders-details?id=65116ffe7244490f95b90e4f")
		       .then().extract().response().asPrettyString();
		System.out.println(res);
	}
